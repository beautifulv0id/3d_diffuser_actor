...
from action_flow.utils.decoder import DiffuserActorIPADecoder
from action_flow.utils.se3_grasp_vector_field import SE3GraspVectorFieldDiffuserActorStyle

from geo3dattn.model.ipa_transformer.ipa_transformer import EfficientInvariantPointAttention, InvariantPointAttention
from geo3dattn.policy.se3_flowmatching.common.se3_flowmatching import RectifiedLinearFlow

class DiffuserActorSE3FlowMatchingEIPA(nn.Module):
    ...
    def __init__(self,
        ...
        encoder = SE3IPAGraspFPSEncoder(
            ...
            attention_module=EfficientInvariantPointAttention,
            point_dim=1
        )
        decoder = DiffuserActorIPADecoder(embedding_dim=embedding_dim, 
                                          nhead=8, 
                                          use_adaln=True,
                                          attention_module=EfficientInvariantPointAttention,
                                          point_dim=1
                                          )
        self.model = SE3GraspVectorFieldDiffuserActorStyle(
            encoder=encoder, 
            decoder=decoder, 

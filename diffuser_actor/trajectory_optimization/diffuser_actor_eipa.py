...
from geo3dattn.model.ipa_transformer.ipa_transformer import EfficientInvariantPointAttention, InvariantPointTransformer
from diffuser_actor.utils.encoder_ipa import EncoderIPA
...
class DiffusionHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.cross_attn = InvariantPointTransformer(
            dim=embedding_dim, depth=2, heads=num_attn_heads, dim_head=embedding_dim//num_attn_heads, kv_dim=embedding_dim,
            point_dim=1, attention_module=EfficientInvariantPointAttention, use_adaln=True
        )
        self.self_attn = InvariantPointTransformer(
            dim=embedding_dim, depth=4, heads=num_attn_heads, dim_head=embedding_dim//num_attn_heads, kv_dim=None,
            point_dim=1, attention_module=EfficientInvariantPointAttention, use_adaln=True
        )
        ...
        if not self.lang_enhanced:
            ...
            dim=embedding_dim, depth=2, heads=num_attn_heads, dim_head=embedding_dim//num_attn_heads, kv_dim=None,
            point_dim=1, attention_module=EfficientInvariantPointAttention, use_adaln=True
        )

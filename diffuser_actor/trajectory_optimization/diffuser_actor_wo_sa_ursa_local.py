...
class DiffuserActorWoSAURSA(nn.Module):
    ...
    def __init__(self,
        ...
        # note: this is just a dummy element to do conversations between local and global frames!
        self.flow = SixDSE3Flow(n_action_steps=1, num_steps=diffusion_timesteps)
    ...
    def forward(
        ...
        # print (noisy_trajectory[cond_mask].shape)
        assert not cond_mask.any()

        # express the noise in the local frame!
        # noisy trajectory is the noise perturbed point:
        noisy_traj_p, noisy_trajectory_r = self.flow._6Dvector_to_trans_rot_mat(noisy_trajectory)
        noise_p, noise_r = self.flow._6Dvector_to_trans_rot_mat(noise)

        d_pt = torch.einsum('...mn,...n->...m', torch.transpose(noisy_trajectory_r, -1, -2), noise_p)
        d_rt = torch.transpose(noisy_trajectory_r, -1, -2) @ noise_r

        # this is the updated noise target, expressed in the local frame of the "noisy" trajectory,...
        noise = self.flow._trans_rot_mat_to_6Dvector(d_pt, d_rt)
        # print ("updated noise shape: ", noise.shape)

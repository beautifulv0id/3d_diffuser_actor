...
id=$(docker run -dt  -v ~/3d_diffuser_actor:/workspace \
    ...
    -v /home/share/3D_attn_felix/peract/raw/test/:/workspace/data/peract/raw/test/ \
...
docker exec -t $id /bin/bash -c "source slurm_scripts/docker/startup-hook.sh &&
                        bash $docker_run_script"
docker stop $id

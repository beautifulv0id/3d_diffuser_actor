...
run_log_dir="diffusion_wosa_nursa_$task_desc-C$C-B$B-lr$lr-DI$dense_interpolation-$interpolation_length-H$num_history-DT$diffusion_timesteps-RN$rot_noise-PN$pos_noise-PCDN$pcd_noise-fps$fps_subsampling_factor"
...
docker exec -t $id /bin/bash -c "source slurm_scripts/startup-hook.sh && cd /workspace/ &&
                        ...
                        --master_port $RANDOM \
                        main_trajectory_wo_sa_nursa.py $kwargs"
docker stop $id

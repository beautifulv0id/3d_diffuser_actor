...
def generate_eval(name) -> str:
    ...
    '#SBATCH --gres=gpu:1',
    '#SBATCH --array=0-0',
    f'#SBATCH --output=eval_logs/slurm_logs/%A_{name}/%a.out',
    f'#SBATCH -J eval_{name}',
    '',
    'checkpoint_dir=$1',
    'seeds=$(echo {0..3})',
    ...
    f'    --output_file eval_logs/{name}/$exp_log_dir/$run_log_dir/seed$seed/$task.json  \\',
...
def main():
    parser = argparse.ArgumentParser(description="Generate eval.sh")   
    parser.add_argument("name", help="Name of the model to evaluate")     
    args = parser.parse_args()

    eval_sh = generate_eval(args.name)
    
    out_file = f"eval_{args.name}.sh"
    with open(f"scripts/slurm/{out_file}", 'w') as f:
        f.write(eval_sh)

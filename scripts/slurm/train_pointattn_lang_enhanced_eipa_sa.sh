...
#SBATCH --gres gpu:4
#SBATCH -C "v100|a5000"
#SBATCH --output=train_logs/slurm_logs/%A_pointattn_lang_enhanced_eipa_sa/%a.out
#SBATCH -J pointattn_lang_enhanced_eipa_sa
...
name=pointattn_lang_enhanced_eipa_sa
...
run_log_dir=pointattn_lang_enhanced_eipa_sa_$task_desc-C$embedding_dim-B$batch_size-lr$lr-H$num_history-DT$diffusion_timesteps-RN$rot_noise-PN$pos_noise-PCDN$pcd_noise-FPS$fps_subsampling_factor-FR$feature_res-ADALN$use_adaln
...
# ============================================================
LOG_DIR_FILE=~/3d_diffuser_actor/train_logs/slurm_logs/${SLURM_ARRAY_JOB_ID}_pointattn_lang_enhanced_eipa_sa/log_dir.txt
...
docker exec -t $id /bin/bash -c "source scripts/slurm/startup-hook.sh && cd /workspace/ &&
    ...
    --master_port $RANDOM \
    main_pointattn_lang_enhanced_eipa_sa.py \
    --tasks ${tasks} \
    --valset ${valset} \
    --dataset ${dataset} \
    --cameras ${cameras} \

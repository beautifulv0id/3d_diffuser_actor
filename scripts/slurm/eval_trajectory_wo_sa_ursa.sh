...
#SBATCH --gres=gpu:1
#SBATCH --array=0-0
#SBATCH --output=eval_logs/slurm_logs/%A_trajectory_wo_sa_ursa/%a.out
#SBATCH -J eval_trajectory_wo_sa_ursa
...
test_model=trajectory_wo_sa_ursa
...
docker exec -t $id /bin/bash -c "source scripts/slurm/startup-hook.sh && cd /workspace/ && \
    ...
    --output_file eval_logs/trajectory_wo_sa_ursa/$exp_log_dir/$run_log_dir/seed$seed/$task.json  \

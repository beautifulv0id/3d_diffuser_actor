...
#SBATCH --gres gpu:4
#SBATCH -C "v100|a5000"
#SBATCH --output=train_logs/slurm_logs/%A_trajectory_flow_matching_eipa/%a.out
#SBATCH -J trajectory_flow_matching_eipa
...
name=trajectory_flow_matching_eipa
...
run_log_dir=trajectory_flow_matching_eipa_$task_desc-C$embedding_dim-B$batch_size-lr$lr-H$num_history-DT$diffusion_timesteps-RN$rot_noise-PN$pos_noise-PCDN$pcd_noise-FPS$fps_subsampling_factor-FR$feature_res
...
# ============================================================
LOG_DIR_FILE=~/3d_diffuser_actor/train_logs/slurm_logs/${SLURM_ARRAY_JOB_ID}_trajectory_flow_matching_eipa/log_dir.txt
...
docker exec -t $id /bin/bash -c "source scripts/slurm/startup-hook.sh && cd /workspace/ &&
    ...
    --master_port $RANDOM \
    main_trajectory_flow_matching_eipa.py \
    --tasks ${tasks} \
    --valset ${valset} \
    --dataset ${dataset} \
    --cameras ${cameras} \

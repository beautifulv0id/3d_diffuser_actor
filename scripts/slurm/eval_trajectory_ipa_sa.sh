...
#SBATCH --gres=gpu:1
#SBATCH --array=0-0
#SBATCH --output=eval_logs/slurm_logs/%A_trajectory_ipa_sa/%a.out
#SBATCH -J eval_trajectory_ipa_sa
...
docker exec -t $id /bin/bash -c "source scripts/slurm/startup-hook.sh && cd /workspace/ && \
    ...
    --output_file eval_logs/trajectory_ipa_sa/$exp_log_dir/$run_log_dir/seed$seed/$task.json  \
